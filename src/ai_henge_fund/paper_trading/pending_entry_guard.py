    ...
    with self._pending_entry_lock:
        ...
        for row in self.execution.list_open_orders():
            if row["symbol"] == symbol:
                self._state.upsert(
                    symbol=symbol, side=side, quantity=float(quantity), entry_price=float(price),
                ...
                self._notify_text(
                    f"ENTRY ALREADY WORKING\n{symbol} {side} qty={int(quantity)} "
...
def _pending_result(self, symbol, order_id, status, price, stop_price, target_price):
    ...
    return MoomooLifecycleResult(
        "PENDING", None, "Moomoo paper entry is still working; duplicate entry blocked",
        broker_order_id=order_id, broker_status=status,
...
def _resolve_pending(self, state):
    ...
    if status.status == FILLED_ALL and status.filled_quantity > 0:
        return _finalize_fill(self, state.symbol, state.side, state.broker_order_id,
                              status.filled_quantity, status.average_price or state.entry_price,
                              state.stop_price, state.target_price)
    if status.status in {"CANCELLED_ALL", "FAILED", "DELETED", "DISABLED", "FILL_CANCELLED"}:
        filled = float(status.filled_quantity or 0)
        if filled > 0:
            return _finalize_fill(self, state.symbol, state.side, state.broker_order_id,
                                  filled, status.average_price or state.entry_price,
                                  state.stop_price, state.target_price, partial=True)
    ...
    _start_pending_watcher(self, state.symbol, state.broker_order_id, state.side,
                           state.quantity, state.entry_price, state.stop_price, state.target_price)
    return _pending_result(self, state.symbol, state.broker_order_id, status.status,
                           state.entry_price, state.stop_price, state.target_price)
...
def _finalize_fill(self, symbol, side, order_id, filled_quantity, fill_price,
                   stop_price, target_price, partial=False):
    _ensure_runtime(self)
    existing = self.positions.get(symbol)
    if existing is not None:
        self._state.upsert(
            symbol=symbol, side=side, quantity=abs(existing.quantity), entry_price=existing.average_price,
    ...
    trade = PaperTrade(
        trade_id=f"moomoo-{order_id}", symbol=symbol, side=side, quantity=qty,
        price=float(fill_price), executed_at=datetime.now(timezone.utc), status=FILLED_ALL,
        metadata={
            "broker": "moomoo", "trading_environment": "SIMULATE", "broker_order_id": order_id,
            "broker_status": "FILLED_PART" if partial else FILLED_ALL,
            "entry_price": float(fill_price), "stop_price": stop_price,
            "target_price": target_price, "position_size_source": "ai",
        },
    )
    self.positions.open_signed(symbol, signed_quantity, float(fill_price),
                               stop_price=stop_price, target_price=target_price)
    self._state.upsert(
        symbol=symbol, side=side, quantity=qty, entry_price=float(fill_price),
        stop_price=stop_price, target_price=target_price, broker_order_id=order_id,
        status="OPEN",
    )
    self._state.record_open(
        trade_id=trade.trade_id, symbol=symbol, side=side, quantity=qty,
        entry_price=float(fill_price), stop_price=stop_price, target_price=target_price,
        broker_entry_order_id=order_id, opened_at=trade.executed_at,
    )
    ...
    from ai_henge_fund.paper_trading.moomoo_lifecycle import MoomooLifecycleResult
    return MoomooLifecycleResult(
        "OPEN", trade, reason, broker_order_id=order_id, broker_status=trade.metadata["broker_status"],
        entry_price=float(fill_price), stop_price=stop_price, target_price=target_price,
    )
...
def _guarded_reconcile_startup(self):
    ...
    try:
        ...
        symbols.update(row["symbol"] for row in self.execution.list_positions())
    except Exception as exc:
